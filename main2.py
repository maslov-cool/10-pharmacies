...
min_lon = min(float(lon), float(min(A, key=lambda i: float(i[0]))[0]))
max_lon = max(float(lon), float(min(A, key=lambda i: float(i[0]))[0]))
min_lat = min(float(lat), float(min(A, key=lambda i: float(i[1]))[1]))
max_lat = max(float(lat), float(min(A, key=lambda i: float(i[1]))[1]))
# работа с картами
...
params = {
    "ll": ",".join([lon, lat]),
    "apikey": api_key,
    'spn': str(max_lon - min_lon) + ',' + str(max_lat - min_lat),
    'pt': ','.join([lon, lat, 'pm']) + 'rdl' + '~' + '~'.join(','.join(i) for i in A)
}
